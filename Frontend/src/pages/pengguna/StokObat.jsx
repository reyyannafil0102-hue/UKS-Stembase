        ...
        <div className="px-6 py-7 sm:px-8 lg:px-10 xl:px-12">
          <div className="flex flex-col gap-3 sm:flex-row">
            <label className="flex h-11 flex-1 items-center gap-3 rounded-lg border border-slate-200 bg-white px-4 focus-within:border-emerald-500 focus-within:ring-4 focus-within:ring-emerald-500/10">
              ...
              <span className="sr-only">Cari nama obat</span>
              <input
                value={search}
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[760px] text-left text-sm">
                  <thead className="border-b border-slate-200 bg-slate-50 text-xs uppercase tracking-wide text-slate-400">
                    <tr>
                      {['No', 'Foto', 'Nama Obat', 'Kegunaan', 'Stok', 'Satuan'].map((heading) => (
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filteredObats.map((obat, index) => (
                      <tr key={obat.id} className="transition hover:bg-slate-50">
                        <td className="px-5 py-4 text-slate-400">{index + 1}</td>
                        <td className="px-5 py-3">
                          {obat.foto ? (
                            <button
                              type="button"
                              onClick={() => setSelectedImage({ src: getObatImageUrl(obat.foto), alt: obat.nama_obat })}
                              ...
                              aria-label={`Perbesar foto ${obat.nama_obat}`}
                            >
                              <img src={getObatImageUrl(obat.foto)} alt={obat.nama_obat} className="h-full w-full object-contain" />
                            </button>
                          ) : (
                        ...
                        <td className="px-5 py-4 font-semibold text-slate-700">{obat.nama_obat}</td>
                        <td className="max-w-md px-5 py-4 text-slate-500">{obat.kegunaan || '-'}</td>
                        <td className={`px-5 py-4 text-base font-bold ${obat.stok > 0 ? 'text-emerald-600' : 'text-red-500'}`}>{obat.stok}</td>
                        <td className="px-5 py-4"><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700">{obat.satuan}</span></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {selectedImage && (
        ...
        >
          ...
          >
            ...
            <img src={selectedImage.src} alt={selectedImage.alt} className="max-h-[84vh] max-w-[85vw] object-contain" />
          </div>
        </div>
      )}
    </div>
