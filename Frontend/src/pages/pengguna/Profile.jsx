...
import { Bell, CalendarDays, CheckCircle2, ClipboardCheck, Edit3, HeartPulse, Home as HomeIcon, Lightbulb, LockKeyhole, LogOut, PackageSearch, Pill, Save, User, UserRound, X } from "lucide-react";
...
export default function Profile() {
	...
	const [profileForm, setProfileForm] = useState({ name: "", email: "", kelas: "", nisn: "", alamat: "" });
	...
	const openEditProfile = () => {
		setProfileForm({ name: user?.name || "", email: user?.email || "", kelas: user?.kelas || "", nisn: user?.nisn || "", alamat: user?.alamat || "" });
		setMessage("");
		setError("");
		setShowEdit(true);
	};
	...
	return (
		<div className="min-h-screen bg-slate-100">
			...
			<MobileMenu currentPath="/profil" onLogout={handleLogout} />

			<main className="min-h-screen lg:ml-64">
				<header className="flex h-20 items-center justify-between border-b border-slate-200 bg-white px-6 sm:px-8 lg:px-10"><div><h2 className="text-2xl font-extrabold text-slate-800">Profil</h2><p className="mt-1 text-sm text-slate-400">Beranda &gt; Profil Saya</p></div><div className="flex items-center gap-5"><button type="button" aria-label="Notifikasi" className="relative flex h-10 w-10 items-center justify-center rounded-full text-slate-500 transition hover:bg-slate-100"><Bell size={18} /><span className="absolute right-2 top-2 h-2 w-2 rounded-full bg-red-500" /></button><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full bg-emerald-100">{user?.foto ? <img src={user.foto} alt="Foto profil" className="h-full w-full object-cover" /> : <UserRound size={19} className="text-emerald-600" />}</div><div className="hidden sm:block"><p className="text-sm font-bold text-slate-700">Hai, {user?.name || "Pengguna"}</p><p className="text-xs text-slate-400">Siswa</p></div></div></div></header>

				<div className="px-6 py-7 sm:px-8 lg:px-10 xl:px-12">
					<section className="relative h-44 overflow-hidden rounded-2xl shadow-sm sm:h-48"><img src={schoolImage} alt="Latar ruang UKS" className="h-full w-full object-cover" /><div className="absolute inset-0 bg-slate-950/65" /><div className="absolute inset-0 flex flex-col items-center justify-center text-white"><div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-full border-4 border-white bg-white shadow-lg">{user?.foto ? <img src={user.foto} alt="Foto profil" className="h-full w-full object-cover" /> : <UserRound size={30} className="text-slate-700" />}</div><h1 className="mt-2 text-lg font-bold">{user?.name || "Pengguna"}</h1><p className="text-xs text-emerald-300">Siswa • {user?.kelas || "Kelas belum diisi"}</p></div></section>

					<div className="mt-5 grid gap-5 xl:grid-cols-2">
						<section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><UserRound size={16} className="text-emerald-600" /><h3 className="text-sm font-bold text-slate-800">Data Diri</h3></div><button type="button" onClick={openEditProfile} className="flex items-center gap-1.5 rounded-lg bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-700 transition hover:bg-emerald-100"><Edit3 size={14} />Edit</button></div><dl className="mt-4 divide-y divide-slate-100 text-xs"><div className="flex justify-between gap-4 py-3"><dt className="text-slate-400">Nama Lengkap</dt><dd className="text-right font-semibold text-slate-700">{user?.name || "-"}</dd></div><div className="flex justify-between gap-4 py-3"><dt className="text-slate-400">Email</dt><dd className="max-w-[65%] break-all text-right font-semibold text-slate-700">{user?.email || "-"}</dd></div><div className="flex justify-between gap-4 py-3"><dt className="text-slate-400">Kelas</dt><dd className="text-right font-semibold text-slate-700">{user?.kelas || "-"}</dd></div><div className="flex justify-between gap-4 py-3"><dt className="text-slate-400">NISN</dt><dd className="text-right font-semibold text-slate-700">{user?.nisn || "-"}</dd></div><div className="flex justify-between gap-4 py-3"><dt className="text-slate-400">Alamat Rumah</dt><dd className="max-w-[65%] text-right font-semibold text-slate-700">{user?.alamat || "-"}</dd></div><div className="flex justify-between gap-4 py-3"><dt className="text-slate-400">Jumlah Kunjungan UKS</dt><dd className="text-right font-semibold text-emerald-600">{kunjungans.length} Kali</dd></div></dl></section>
